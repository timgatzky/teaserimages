...
$GLOBALS['TL_DCA']['tl_news']['config']['onload_callback'][]=array('tl_news_teaserimage', 'modifyPalette');
 ...
 * Subpalettes
 */
array_insert($GLOBALS['TL_DCA']['tl_news']['subpalettes'], 1, array
(
...
class tl_news_teaserimage extends Backend
{
	public function modifyPalette()
	{
		// Check if TinyMCE should be used
		if($GLOBALS['TL_CONFIG']['teaser_news_tinymce'])
		{
			...
			(
				...
				'search'                  => true,
				'inputType'               => 'textarea',
				'eval'                    => array('rte'=>'tinyMCE', 'helpwizard'=>false),
			);
		}
		...
		if (version_compare(VERSION . '.' . BUILD, '2.11.0', '<'))
      	{
      		$GLOBALS['TL_DCA']['tl_news']['fields']['teaser_size'] = array
			(
				'label'                   => &$GLOBALS['TL_LANG']['tl_content']['size'],
				'exclude'                 => false,
				'inputType'               => 'imageSize',
